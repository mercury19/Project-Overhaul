...
# Refresh Merchants (used to be a regular trigger)
  (24 * 7,
   [
    ...
    (call_script, "script_refresh_locations"),
  ]),
...
#Party AI: pruning some of the prisoners in each center (once a week)
  ...
  #Converging center prosperity to ideal prosperity; moved up from a 24 x 15 slot to a 24 slot  
  (24,
   [
     (try_for_range, ":center_no", centers_begin, centers_end),
  ...
  # Give some xp to hero parties
   ...
   [      
       (try_for_range, ":troop_no", active_npcs_begin, active_npcs_end),
         ...
         (store_skill_level, ":trainer_level", skl.trainer, ":troop_no"),                  
  ...
  (0.1,
   [
	   ...
	   (try_for_parties, ":bandit_camp"),
		 ...
		 (ge, ":template", "pt_plains_bandit_lair"),
   ...
   [
    ...
    (try_begin),
      ...
      (try_for_range, ":cur_edible", food_begin, food_end),      
        (call_script, "script_cf_player_has_item_without_modifier", ":cur_edible", imod.rotten),
    ...
    (try_for_range, ":unused", 0, ":consumption_amount"),
      ...
      (try_for_range, ":cur_food", food_begin, food_end),
        ...
        (call_script, "script_cf_player_has_item_without_modifier", ":cur_food", imod.rotten),
  ...
  (24,
   [
     ...
     (try_for_range, ":i_slot", 0, ":inv_size"),
       ...
       (try_begin),
         (ge, ":modifier", imod.fresh),
         (lt, ":modifier", imod.rotten),
         (val_add, ":modifier", 1),
         (troop_set_inventory_slot_modifier, "trp_player", ":i_slot", ":modifier"),
       (else_try),
         (lt, ":modifier", imod.fresh),
         (troop_set_inventory_slot_modifier, "trp_player", ":i_slot", imod.fresh),
       (try_end),
     (try_end),
    ]),
 ...
 #Update how good a target player is for bandits
  ...
  (12,
   [(try_for_range, ":center_no", centers_begin, centers_end),
      ...
      (party_get_slot, ":cur_lvl", ":center_no", ":cur_improvement"),
      (val_add, ":cur_lvl", 1),
      (party_set_slot, ":center_no", ":cur_improvement", ":cur_lvl"),
   ...
   [(assign, ":num_active_tournaments", 0),
    (try_for_range, ":center_no", towns_begin, towns_end),
      ...
      (try_begin),
        ...
        (try_begin),
          (str_store_troop_name_link, s1, reg0),
          (str_store_party_name_link, s2, ":center_no"),
          (display_message, "@{s1} has won the tournament at {s2}."),
          (troop_is_hero, reg0),
          (call_script, "script_change_troop_renown", reg0, 20),
        (try_end),
      (try_end),
...
[
  ...
  (1,
   [(neg|map_free),
	...
	(try_begin),
      ...
      (assign, ":must_pay", 1),
	(else_try),
      (is_between, "$g_last_rest_center", "p_zendar", "p_dhorak_keep"),
      (assign, ":must_pay", 1),
	(else_try),
      (eq, "$g_last_rest_center", "p_dhorak_keep"),
  ...
  (36,
   [
       (call_script, "script_update_party_creation_random_limits"), # Make parties larger as game progresses.
       (call_script, "script_spawn_bandits"),
       (call_script, "script_spawn_bandit_lairs"), # Now a separate script
    ]),
	
  # Spawn other parties.
  (6,
   [(store_num_parties_of_template, ":manhunters", "pt_manhunters"),  
	(try_begin),  
		(lt, ":manhunters", 4),
		(store_add, ":p.town_22_plus_one", "p_town_22", 1),
		(store_random_in_range, ":selected_town", "p_town_1", ":p.town_22_plus_one"),
		(call_script, "script_spawn_patrol", "pt_manhunters", ":selected_town", 45),
	(else_try),
		(lt, ":manhunters", 6),
		(party_is_active, "p_zendar"),
		(call_script, "script_spawn_patrol", "pt_manhunters", "p_zendar", 45),
	(try_end),
	(store_num_parties_of_template, ":slavers", "pt_slavers"),  
	(try_begin),
		(lt, ":slavers", 3),
		(party_is_active, "p_salt_mine"),
		(call_script, "script_spawn_patrol", "pt_slavers", "p_salt_mine", 25),
	(try_end),
	(store_num_parties_of_template, ":sword_sisters", "pt_sword_sisters"),  
	(try_begin),
		(lt, ":sword_sisters", 3),
		(party_is_active, "p_dhorak_keep"),
		(call_script, "script_spawn_patrol", "pt_sword_sisters", "p_dhorak_keep", 45),
	(try_end),
    ]),
   ...
   [
    ...
    (try_for_range, ":cur_kingdom", kingdoms_begin, kingdoms_end),
      ...
      (try_begin),
		...
		(faction_set_note_available, ":cur_kingdom", 0),
      (try_end),
    (try_end),
    (try_begin),
  ...
  (3, #check to see if player's court has been captured
   [
     ...
     (try_for_parties, ":bandit_party"),
       ...
       (is_between, ":bandit_party_template", "pt_plains_bandits", "pt_deserters"),
       ...
       (else_try), #Otherwise, act freely
         ...
         (try_begin),
           ...
           (party_template_get_slot, ":spawnpoint_begin", ":bandit_party_template", slot_party_template_lair_spawnpoint_begin),
           (party_template_get_slot, ":spawnpoint_end", ":bandit_party_template", slot_party_template_lair_spawnpoint_end),
           (store_random_in_range, ":spawnpoint", ":spawnpoint_begin", ":spawnpoint_end"),
       ...
       (try_begin),
         ...
         (try_for_range, ":unused", 0, 20), #Repeat trial twenty times
           ...
           (else_try),
             ...
             (try_begin),
               (assign, ":end_cond", 100),
               (assign, "$qst.scout_waypoints_wp_1", -1),
               (assign, "$qst.scout_waypoints_wp_2", -1),
               (assign, "$qst.scout_waypoints_wp_3", -1),
               (assign, ":continue", 0),
               (try_for_range, ":unused", 0, ":end_cond"),
                 (try_begin),
                   (lt, "$qst.scout_waypoints_wp_1", 0),
                   (call_script, "script_cf_get_random_enemy_center_within_range", ":faction_marshall_party", 50),
                   (assign, "$qst.scout_waypoints_wp_1", reg0),
                 (try_end),
                 (try_begin),
                   (lt, "$qst.scout_waypoints_wp_2", 0),
                   (call_script, "script_cf_get_random_enemy_center_within_range", ":faction_marshall_party", 50),
                   (neq, "$qst.scout_waypoints_wp_1", reg0),
                   (assign, "$qst.scout_waypoints_wp_2", reg0),
                 (try_end),
                 (try_begin),
                   (lt, "$qst.scout_waypoints_wp_3", 0),
                   (call_script, "script_cf_get_random_enemy_center_within_range", ":faction_marshall_party", 50),
                   (neq, "$qst.scout_waypoints_wp_1", reg0),
                   (neq, "$qst.scout_waypoints_wp_2", reg0),
                   (assign, "$qst.scout_waypoints_wp_3", reg0),
                 (try_end),
                 (neq, "$qst.scout_waypoints_wp_1", "$qst.scout_waypoints_wp_2"),
                 (neq, "$qst.scout_waypoints_wp_1", "$qst.scout_waypoints_wp_2"),
                 (neq, "$qst.scout_waypoints_wp_2", "$qst.scout_waypoints_wp_3"),
                 (ge, "$qst.scout_waypoints_wp_1", 0),
                 (ge, "$qst.scout_waypoints_wp_2", 0),
                 (ge, "$qst.scout_waypoints_wp_3", 0),
                 (assign, ":end_cond", 0),
                 (assign, ":continue", 1),
               (try_end),
               (eq, ":continue", 1),
               (assign, "$qst.scout_waypoints_wp_1_visited", 0),
               (assign, "$qst.scout_waypoints_wp_2_visited", 0),
               (assign, "$qst.scout_waypoints_wp_3_visited", 0),
...
# Train peasants against bandits
  (1,
   [
     ...
     (eq, "$qst.train_peasants_against_bandits_currently_training", 1),
     (val_add, "$qst.train_peasants_against_bandits_num_hours_trained", 1),
     (call_script, "script_get_max_skill_of_player_party", "skl_trainer"),
     (assign, ":trainer_skill", reg0),
     (store_sub, ":needed_hours", 20, ":trainer_skill"),
     (val_mul, ":needed_hours", 3),
     (val_div, ":needed_hours", 5),
     (ge, "$qst.train_peasants_against_bandits_num_hours_trained", ":needed_hours"),
     (assign, "$qst.train_peasants_against_bandits_num_hours_trained", 0),
     (rest_for_hours, 0, 0, 0), #stop resting
...
# Scout waypoints
  (1,
   [
     ...
     (try_begin),
       (eq, "$qst.scout_waypoints_wp_1_visited", 0),
       (store_distance_to_party_from_party, ":distance", "$qst.scout_waypoints_wp_1", "p_main_party"),
       (le, ":distance", 3),
       (assign, "$qst.scout_waypoints_wp_1_visited", 1),
       (str_store_party_name_link, s1, "$qst.scout_waypoints_wp_1"),
       (display_message, "@{s1} is scouted."),
     (try_end),
     (try_begin),
       (eq, "$qst.scout_waypoints_wp_2_visited", 0),
       (store_distance_to_party_from_party, ":distance", "$qst.scout_waypoints_wp_2", "p_main_party"),
       (le, ":distance", 3),
       (assign, "$qst.scout_waypoints_wp_2_visited", 1),
       (str_store_party_name_link, s1, "$qst.scout_waypoints_wp_2"),
       (display_message, "@{s1} is scouted."),
     (try_end),
     (try_begin),
       (eq, "$qst.scout_waypoints_wp_3_visited", 0),
       (store_distance_to_party_from_party, ":distance", "$qst.scout_waypoints_wp_3", "p_main_party"),
       (le, ":distance", 3),
       (assign, "$qst.scout_waypoints_wp_3_visited", 1),
       (str_store_party_name_link, s1, "$qst.scout_waypoints_wp_3"),
       (display_message, "@{s1} is scouted."),
     (try_end),
     (eq, "$qst.scout_waypoints_wp_1_visited", 1),
     (eq, "$qst.scout_waypoints_wp_2_visited", 1),
     (eq, "$qst.scout_waypoints_wp_3_visited", 1),
...
# Kill local merchant
  ...
  (3, [(neg|map_free),
       ...
       (rest_for_hours, 0, 0, 0), #stop resting
       (assign, "$auto_enter_town", "$qst.kill_local_merchant_center"),
...
# Collect taxes
  (1, [(neg|map_free),
       ...
       (eq, "$g_player_is_captive", 0),
       (eq, "$qst.collect_taxes_currently_collecting", 1),
       ...
       (else_try),
         ...
         (assign, ":max_collected_tax", "$qst.collect_taxes_hourly_income"),
       ...
       (try_begin),
         ...
         (val_sub, "$qst.collect_taxes_menu_counter", 1),
         (le, "$qst.collect_taxes_menu_counter", 0),
       ...
       (else_try), #Chance of revolt against player
         ...
         (val_sub, "$qst.collect_taxes_unrest_counter", 1),
         (le, "$qst.collect_taxes_unrest_counter", 0),
         (eq, "$qst.collect_taxes_halve_taxes", 0),
         (quest_set_slot, "qst_collect_taxes", slot_quest_current_state, 3),

         (store_div, ":unrest_chance", 10000, "$qst.collect_taxes_total_hours"),
...
(1, 
  ...
  (1,
   [
     ...
     (store_add, ":fac.kingdom_6_plus_one", "fac_kingdom_6", 1),

     (try_for_range, ":faction_1", "fac_kingdom_1", ":fac.kingdom_6_plus_one"),
       (try_for_range, ":faction_2", "fac_kingdom_1", ":fac.kingdom_6_plus_one"),
