...
#     The prefix mt. is automatically added before each mission-template id
#
...
#used for torunament master scene
  ...
  (
    "cellar_fistfight",mtf_arena_fight,-1,
    ...
    [
      ...
      (0, 0, ti_once, [],
       [
		 ...
		 (try_for_agents, ":agent"),
		   ...
		   (agent_set_animation_progress, ":agent", reg0), # Desync their standing animation!
		 (try_end),
         ]),
      ...
      (ti_on_agent_hit, 0, 0, [
			...
			(lt, "$fistfight_points_opponent", 5), # Don't show or count score when the match is already won
			(try_begin),
			  (eq,":player_agent",":attacker_agent"),
			  (agent_get_animation, ":cur_anim", ":player_agent", 0),
			  (this_or_next|eq, ":cur_anim", "anim_kick_right_leg"),
			  (eq, ":cur_anim", "anim_kick_left_leg"), # Check for kicks; they earn you a warning, then disqualification.
			  (assign, ":kick", 1),
			(else_try),
			  (eq,":player_agent",":attacker_agent"),
			...
			(else_try),
			  ...
			  (val_add, "$fistfight_bets", "$fistfight_points_opponent"),
			  (val_sub, "$fistfight_bets", "$fistfight_points_player"),
			  (store_attribute_level, ":strength", "$fistfight_opponent", ca_strength),
			  (store_skill_level, ":power_strike", "skl_power_strike", "$fistfight_opponent"),
			(try_end),
			(try_begin),
			  (eq, ":kick", 1),
			  (assign, ":damage", 0), # Kicking while blocking is a bit of an exploit; this at least prevents kicks from dealing damage.
	  ...
	  (eq, "$fistfight_result", 0)],
       [
		   ...
		   (store_attribute_level, ":opponent_strength", "$fistfight_opponent", ca_strength),
    ...
    [
      ...
      (0, 0, ti_once, 
       ...
       [
         ...
         (try_begin),
		   (store_sub, ":troop_of_merchant", ":starting_town_culture", "fac_culture_1"),
           (val_add, ":troop_of_merchant", startup_merchants_begin),
           (is_between, ":troop_of_merchant", startup_merchants_begin, startup_merchants_end),
         (else_try),  
