...
# Runaway Peasants quest
  (0.2, 0.0, 0.0,
   [
       ...
       (try_begin),
         (party_is_active, "$qst.bring_back_runaway_serfs_party_1"),
         (try_begin),
           (party_is_in_town, "$qst.bring_back_runaway_serfs_party_1", ":quest_target_center"),
           (remove_party, "$qst.bring_back_runaway_serfs_party_1"),
           (val_add, "$qst.bring_back_runaway_serfs_num_parties_fleed", 1),
         (else_try),
           (party_is_in_town, "$qst.bring_back_runaway_serfs_party_1", ":quest_object_center"),
           (remove_party, "$qst.bring_back_runaway_serfs_party_1"),
           (val_add, "$qst.bring_back_runaway_serfs_num_parties_returned", 1),
         (else_try),
           (store_distance_to_party_from_party, ":cur_distance", "p_main_party", "$qst.bring_back_runaway_serfs_party_1"),
           (gt, ":cur_distance", 3),
           (party_set_ai_object, "$qst.bring_back_runaway_serfs_party_1", ":quest_target_center"),
         (try_end),
       (try_end),
       (try_begin),
         (party_is_active, "$qst.bring_back_runaway_serfs_party_2"),
         (try_begin),
           (party_is_in_town, "$qst.bring_back_runaway_serfs_party_2", ":quest_target_center"),
           (remove_party, "$qst.bring_back_runaway_serfs_party_2"),
           (val_add, "$qst.bring_back_runaway_serfs_num_parties_fleed", 1),
         (else_try),
           (party_is_in_town, "$qst.bring_back_runaway_serfs_party_2", ":quest_object_center"),
           (remove_party, "$qst.bring_back_runaway_serfs_party_2"),
           (val_add, "$qst.bring_back_runaway_serfs_num_parties_returned", 1),
         (else_try),
           (store_distance_to_party_from_party, ":cur_distance", "p_main_party", "$qst.bring_back_runaway_serfs_party_2"),
           (gt, ":cur_distance", 3),
           (party_set_ai_object, "$qst.bring_back_runaway_serfs_party_2", ":quest_target_center"),
         (try_end),
       (try_end),
       (try_begin),
         (party_is_active, "$qst.bring_back_runaway_serfs_party_3"),
         (try_begin),
           (party_is_in_town, "$qst.bring_back_runaway_serfs_party_3", ":quest_target_center"),
           (remove_party, "$qst.bring_back_runaway_serfs_party_3"),
           (val_add, "$qst.bring_back_runaway_serfs_num_parties_fleed", 1),
         (else_try),
           (party_is_in_town, "$qst.bring_back_runaway_serfs_party_3", ":quest_object_center"),
           (remove_party, "$qst.bring_back_runaway_serfs_party_3"),
           (val_add, "$qst.bring_back_runaway_serfs_num_parties_returned", 1),
         (else_try),
           (store_distance_to_party_from_party, ":cur_distance", "p_main_party", "$qst.bring_back_runaway_serfs_party_3"),
           (gt, ":cur_distance", 3),
           (party_set_ai_object, "$qst.bring_back_runaway_serfs_party_3", ":quest_target_center"),
         (try_end),
       (try_end),
       (assign, ":sum_removed", "$qst.bring_back_runaway_serfs_num_parties_returned"),
       (val_add, ":sum_removed", "$qst.bring_back_runaway_serfs_num_parties_fleed"),
       (ge, ":sum_removed", 3),
       (try_begin),
         (ge, "$qst.bring_back_runaway_serfs_num_parties_returned", 3),
         (call_script, "script_succeed_quest", "qst_bring_back_runaway_serfs"),
       (else_try),
         (eq, "$qst.bring_back_runaway_serfs_num_parties_returned", 0),
...
# Follow Spy quest
  (0.5, 0.0, 0.0,
   [
       ...
       (eq, "$qst.follow_spy_no_active_parties", 0),
       ...
       (try_begin),
         (this_or_next|ge, "$qst.follow_spy_run_away", 2),
         (this_or_next|neg|party_is_active, "$qst.follow_spy_spy_party"),
         (neg|party_is_active, "$qst.follow_spy_spy_partners_party"),
       (else_try),
         (eq, "$qst.follow_spy_meeting_state", 0),
         (store_distance_to_party_from_party, ":cur_distance", "p_main_party", "$qst.follow_spy_spy_party"),
         (try_begin),
           ...
           (val_add, "$qst.follow_spy_run_away", 1),
           (try_begin),
             (eq, "$qst.follow_spy_run_away", 2),
             (assign, ":abort_meeting", 1),
             (display_message, "str_qst_follow_spy_noticed_you"),
           (try_end),
         (else_try),
           (store_distance_to_party_from_party, ":cur_distance", "$qst.follow_spy_spy_partners_party", "$qst.follow_spy_spy_party"),
           (le, ":cur_distance", 1),
           (party_attach_to_party, "$qst.follow_spy_spy_party", "$qst.follow_spy_spy_partners_party"),
           (assign, "$qst.follow_spy_meeting_state", 1),
           (assign, "$qst.follow_spy_meeting_counter", 0),
         (try_end),
       (else_try),
         (eq, "$qst.follow_spy_meeting_state", 1),
         (store_distance_to_party_from_party, ":cur_distance", "p_main_party", "$qst.follow_spy_spy_partners_party"),
         (try_begin),
           (le, ":cur_distance", 1),
           (party_detach, "$qst.follow_spy_spy_party"),
           (val_add, "$qst.follow_spy_run_away", 1),
           (try_begin),
             (eq, "$qst.follow_spy_run_away", 2),
             (assign, ":abort_meeting", 1),
           ...
           (try_end),
         (else_try),
           (val_add, "$qst.follow_spy_meeting_counter", 1),
           (gt, "$qst.follow_spy_meeting_counter", 4),
           (party_detach, "$qst.follow_spy_spy_party"),
           (assign, ":abort_meeting", 1),
           (assign, "$qst.follow_spy_meeting_state", 2),
         (try_end),
       (try_end),
       (try_begin),
         (eq, ":abort_meeting", 1),
         (party_set_ai_object, "$qst.follow_spy_spy_party", ":quest_giver_center"),
         
         (party_set_ai_object, "$qst.follow_spy_spy_partners_party", ":quest_object_center"),
         
         (party_set_ai_behavior, "$qst.follow_spy_spy_party", ai_bhvr_travel_to_party),
         (party_set_ai_behavior, "$qst.follow_spy_spy_partners_party", ai_bhvr_travel_to_party),
       ...
       (assign, ":num_active", 0),
       (try_begin),
         (party_is_active, "$qst.follow_spy_spy_party"),
         (val_add, ":num_active", 1),
         (party_is_in_town, "$qst.follow_spy_spy_party", ":quest_giver_center"),
         (remove_party, "$qst.follow_spy_spy_party"),
         (assign, "$qst.follow_spy_spy_back_in_town", 1),
         (val_sub, ":num_active", 1),
       (try_end),
       (try_begin),
         (party_is_active, "$qst.follow_spy_spy_partners_party"),
         (val_add, ":num_active", 1),
         (party_is_in_town, "$qst.follow_spy_spy_partners_party", ":quest_object_center"),
         (remove_party, "$qst.follow_spy_spy_partners_party"),
         (assign, "$qst.follow_spy_partner_back_in_town", 1),
         (val_sub, ":num_active", 1),
       (try_end),
       (try_begin),
         (eq, "$qst.follow_spy_partner_back_in_town",1),
         (eq, "$qst.follow_spy_spy_back_in_town",1),
         (call_script, "script_fail_quest", "qst_follow_spy"),
       (try_end),
       (try_begin),
         (eq, ":num_active", 0),
         (assign, "$qst.follow_spy_no_active_parties", 1),
...
# Troublesome bandits
  (0.3, 0.0, 1.1, [(check_quest_active, "qst_troublesome_bandits"),
                   ...
                   (lt, "$qst.troublesome_bandits_eliminated", ":cur_eliminated"),
                   (store_num_parties_destroyed_by_player, ":cur_eliminated_by_player", "pt_troublesome_bandits"),
                   (eq, ":cur_eliminated_by_player", "$qst.troublesome_bandits_eliminated_by_player"),
                   ],
                  [(display_message, "str_bandits_eliminated_by_another"),
  ...
  (0.3, 0.0, 1.1, [(check_quest_active, "qst_troublesome_bandits"),
                   ...
                   (lt, "$qst.troublesome_bandits_eliminated", ":cur_eliminated"),
                   (store_num_parties_destroyed_by_player, ":cur_eliminated_by_player", "pt_troublesome_bandits"),
                   (neq, ":cur_eliminated_by_player", "$qst.troublesome_bandits_eliminated_by_player"),
                   ],
